    ...
    <PageLayout home>
      <div className='container px-3 pb-[20px] md:mt-[50px] pt-20 md:pt-0'>
        <Text title className="text-3xl">
          Le vie per l'Australia
        </Text>
        <hr className="mt-5" />
        <Text className="mt-10">
        Incontro del 6 Aprile 2017 presso la Biblioteca di Pordenone.
        </Text>
...
export async function getStaticProps() {
  const dir_slider_img='./public/imp_assets/meetings/le_vie_per_australia_slider'
  ...
  const filenames = fs.readdirSync(absolute_path_dir, { withFileTypes: true })
    ...
    .map(file => '/public/imp_assets/meetings/le_vie_per_australia_slider/' + file.name)
  return {
    props: {
      filenames,
    },
